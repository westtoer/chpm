...
//            "contents": "<img src='http://intranet.westtoer.be/sites/default/files/westtoer-logo.jpg'>"
    ...
    pdf.create(html, opts).toFile(path, function (err, res) {
        if (err) {
            console.error("error poducing pdf at " + path + "\n" + err);
            return;
        } //else
        cb(res);
    });
...
function mailpdf(email, body, path) {
}

function processData(inputFile, cb) {
    var data = {
...
function runCommand(cmd) {
    ...
    processData(dataFile, function (data) {
        var html, pdfFile = path.join(outputDir, String(data.nota_nr) + ".pdf");
